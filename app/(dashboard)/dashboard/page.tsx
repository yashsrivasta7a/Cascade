      ...
      <div className="shrink-0 h-14 px-6 flex items-center justify-between border-b border-[#6b6b6b] dark:border-zinc-800/60">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => refetch()}
            disabled={isFetching}
            className="h-8 px-3 text-xs font-medium text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white bg-gray-100 dark:bg-zinc-800/50 hover:bg-gray-200 dark:hover:bg-zinc-800 border border-[#6b6b6b] dark:border-zinc-700/50 rounded-lg flex items-center gap-1.5 transition-all"
          >
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
                ...
                <div className={cn(
                  ...
                  <div className="relative">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <ArrowUpRight className="w-4 h-4 text-gray-500 dark:text-zinc-600" />
                    </div>
                    <p className="text-xs text-gray-500 dark:text-zinc-500 mb-1">{config.label}</p>
                    <p className="text-2xl font-semibold text-gray-900 dark:text-white mb-1">{stat?.value ?? "—"}</p>
                    <p className="text-xs">
                      <span className={stat?.change?.startsWith("+") ? "text-emerald-600 dark:text-emerald-400" : stat?.change?.startsWith("-") ? "text-red-600 dark:text-red-400" : "text-gray-500 dark:text-zinc-500"}>
                        {stat?.change ?? "—"}
                      </span>{" "}
                      <span className="text-gray-500 dark:text-zinc-600">{stat?.changeLabel ?? ""}</span>
                    </p>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="grid grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-zinc-900/50 border border-[#6b6b6b] dark:border-zinc-800/60 rounded-xl overflow-hidden shadow-md dark:shadow-none">
              <div className="p-5 border-b border-[#6b6b6b] dark:border-zinc-800/60 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-zinc-500 mt-0.5">Latest workflow executions</p>
                </div>
                <Link href="/executions">
                  <button className="text-xs text-gray-500 dark:text-zinc-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
                    View All →
                  </button>
                </Link>
              </div>
              ...
              <div className="divide-y divide-[#6b6b6b] dark:divide-zinc-800/50">
                {recentActivity.length === 0 ? (
                  <div className="px-5 py-12 text-center text-gray-500 dark:text-zinc-500 text-sm">
                  ...
                  recentActivity.map((activity: { id: string; workflow: string; status: string; time: string; duration?: string }, i: number) => (
                    <motion.div
                      ...
                      className="px-5 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-zinc-800/30 transition-colors"
                    >
                      <div className="flex items-center gap-4">
                        <div className={cn(
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-900 dark:text-white">{activity.workflow}</p>
                          <p className="text-xs text-gray-500 dark:text-zinc-500">{activity.time}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className={cn(
                        ...
                        {activity.duration && (
                          <span className="text-xs text-gray-500 dark:text-zinc-500 w-12 text-right font-mono">{activity.duration}</span>
                        )}
                      </div>
                    </motion.div>
                  ))
          ...
          >
            ...
            {/* Active Workflows */}
            <div className="p-5 bg-white dark:bg-zinc-900/50 border border-[#6b6b6b] dark:border-zinc-800/60 rounded-xl shadow-md dark:shadow-none">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">Active Workflows</h2>
              <div className="flex items-center gap-2 mb-3">
                {activeWorkflows > 0 && (
              ...
              <Link href="/executions" className="block">
                <button className="w-full h-9 px-4 text-xs font-medium text-gray-700 dark:text-white bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 border border-[#6b6b6b] dark:border-zinc-700/50 rounded-lg flex items-center justify-center gap-2 transition-colors">
            ...
            <div className="p-5 bg-white dark:bg-zinc-900/50 border border-[#6b6b6b] dark:border-zinc-800/60 rounded-xl group hover:border-amber-300 dark:hover:border-amber-500/20 transition-colors shadow-md dark:shadow-none">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-gray-900 dark:text-white">AI Credits</h2>
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between text-sm mb-1">
                  ...
                  <span className="text-gray-500 dark:text-zinc-500">{100 - creditPercent}%</span>
                </div>
              ...
              <p className="text-[11px] text-gray-500 dark:text-zinc-600">of {credits.total.toLocaleString()} credits</p>
            </div>
          </motion.div>
        </div>
      </div>
