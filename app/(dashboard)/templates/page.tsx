...
const templates: Template[] = [
 { id: "t1", name: "AI Video Generator", description: "Generate videos from text prompts with voice narration", category: "multimodal", nodes: 6, estimatedCost: 48, estimatedDuration: "3-5 min", author: "Cascade", downloads: 12500, rating: 4.9, featured: true, tags: ["video", "voice", "script"] },
 { id: "t2", name: "Product Photo Enhancement", description: "Upscale, relight, and remove backgrounds from product photos", category: "image", nodes: 4, estimatedCost: 15, estimatedDuration: "30 sec", author: "Cascade", downloads: 8200, rating: 4.8, tags: ["ecommerce", "photos", "enhancement"] },
 ...
 { id: "t4", name: "Blog Post Generator", description: "Generate SEO-optimized blog posts with images from a topic", category: "llm", nodes: 5, estimatedCost: 12, estimatedDuration: "45 sec", author: "Cascade", downloads: 15600, rating: 4.6, featured: true, tags: ["content", "seo", "blog"] },
 ...
 { id: "t7", name: "Voice Cloning & Dubbing", description: "Clone a voice and use it to dub videos in different languages", category: "multimodal", nodes: 7, estimatedCost: 55, estimatedDuration: "5-8 min", author: "Cascade", downloads: 2100, rating: 4.8, tags: ["voice", "dubbing", "localization"] },
...
export default function TemplatesPage() {
 ...
 <PageBackground>
 {/* Header */}
 <div className="shrink-0 h-14 px-6 flex items-center justify-between border-b border-blue-100 dark:border-zinc-800/60">
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded-lg bg-white dark:bg-zinc-800/50 flex items-center justify-center">
 <Layers className="w-4 h-4 text-slate-800 dark:text-zinc-400" />
 </div>
 <div>
 <h1 className="text-sm font-semibold text-slate-900 dark:text-white">Templates</h1>
 <p className="text-[11px] text-slate-700 dark:text-zinc-500">Pre-built workflows</p>
 </div>
 </div>
 <div className="flex items-center gap-3">
 ...
 <button className="text-xs text-slate-700 hover:text-white flex items-center gap-1">
 ...
 <p className="text-xs text-slate-700 mb-3 line-clamp-2">{template.description}</p>
 <div className="flex items-center gap-3 text-[11px] text-slate-700 mb-3">
 ...
 <span className="text-[10px] text-slate-700">({template.downloads.toLocaleString()})</span>
 </div>
 ...
 className="w-56 h-9 pl-9 pr-4 text-sm bg-zinc-900/50 border border-zinc-800 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:border-zinc-700"
 />
 </div>
 ...
 selectedCategory === cat ? "bg-zinc-800 text-white" : "text-slate-700 hover:text-slate-700"
 )}
 >
 {categoryIcons[cat]}
 {cat === "llm" ? "LLM" : cat}
 ...
 <button className="p-2 rounded-lg text-slate-700 hover:text-red-400 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all">
 <Heart className="w-4 h-4" />
 </button>
 </div>

 <h3 className="font-medium text-white mb-1 group-hover:text-blue-400 transition-colors">{template.name}</h3>
 <p className="text-xs text-slate-700 mb-3 line-clamp-2 flex-1">{template.description}</p>

 <div className="flex flex-wrap gap-1.5 mb-3">
 {template.tags.map((tag) => (
 <span key={tag} className="px-2 py-0.5 rounded-full text-[10px] bg-zinc-800 text-slate-600">{tag}</span>
 ))}
 </div>

 <div className="flex items-center justify-between text-[11px] text-slate-700 mb-3 pt-3 border-t border-zinc-800/60">
