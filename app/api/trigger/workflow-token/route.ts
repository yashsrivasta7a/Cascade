...
// =============================================================================
// WORKFLOW TRIGGER TOKEN ENDPOINT
// =============================================================================
// Creates a one-time trigger token for the execute-workflow task.
// This token allows the frontend to trigger the workflow directly using
// Trigger.dev's React hooks (useRealtimeTaskTrigger).
// =============================================================================

export const maxDuration = 60;

interface TriggerTokenRequest {
  workflowId: string;
  nodes: Node[];
  edges: Edge[];
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: TriggerTokenRequest = await request.json();
    ...
    // Create a trigger token for the execute-workflow task
    // This token can only be used once to trigger the task
    const triggerToken = await triggerAuth.createTriggerPublicToken("execute-workflow");

    console.log(`[WorkflowToken] Created trigger token for workflow ${workflowExecution.id}`);
    ...
    console.error("[WorkflowToken] Error:", error);
    return NextResponse.json({
