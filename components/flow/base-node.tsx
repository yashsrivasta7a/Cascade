...
function BaseNodeComponent({
 ...
 // #161616 sits almost on the canvas ground, so cards had to rely on shadow
 // alone to separate. Lifting the surface lets the card read as a raised plane
 // and lets the shadow do less work.
 const nodeBgColor = isDarkMode ? "#1c1c1e" : "#ffffff";
 ...
 style={{
 // A hard 0.4-alpha shadow on every card stacks into visual mud once a
 // dozen nodes overlap. Two soft layers read as depth instead of grime,
 // and the running state lifts rather than glows.
 boxShadow: isActuallyRunning
 ? `0 0 0 1px ${accentColor}55, 0 8px 24px -6px rgba(0,0,0,0.45), 0 2px 6px -2px rgba(0,0,0,0.3)`
 : isSelected
 ? "0 10px 30px -8px rgba(0,0,0,0.5), 0 2px 6px -2px rgba(0,0,0,0.3)"
 : "0 4px 14px -6px rgba(0,0,0,0.4), 0 1px 3px -1px rgba(0,0,0,0.25)",
 ...
 />
 {/* Category accent. A 4px bar across the full card width made colour the
 first thing read on every node; at a dozen nodes that is a stripe
 chart, not a canvas. A short inset rule carries the same identity as
 a mark rather than a banner. */}
 <rect
 x={14}
 y={0}
 width={34}
 ...
 <div className="w-5 h-5 rounded-full flex items-center justify-center text-slate-800 dark:text-zinc-600 hover:text-slate-700 dark:hover:text-slate-700 cursor-default transition-colors">
 ...
 <div className={cn("rounded-lg p-3 space-y-2 border", isDarkMode ? "bg-[#0f0f0f] border-zinc-800/30" : "bg-white border-blue-100")}>
 {left ?? children ?? (
 <div className="text-[11px] text-slate-700 dark:text-zinc-600 italic" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
 ...
 {right && (
 <div>
 <div className="text-[10px] text-slate-800 dark:text-zinc-500 mb-1.5" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
 Output
 </div>
 <div className={cn("rounded-lg p-3 border", isDarkMode ? "bg-[#0f0f0f] border-zinc-800/30" : "bg-white border-gray-400")}>
 {right}
 </div>
 </div>
 )}
 ...
 "bg-white dark:bg-[#1a1a1a] border border-blue-100 dark:border-white/10",
 "text-[11px] text-slate-700 dark:text-white/90 whitespace-nowrap",
 ...
 "bg-white dark:bg-zinc-900 border border-blue-100 dark:border-white/10",
 ...
 isUploading
 ? "bg-zinc-900 text-slate-700 cursor-not-allowed border border-zinc-800"
 : status === "running"
 ...
 isRunningOrQueued
 ? "bg-zinc-900 text-slate-700 cursor-not-allowed border border-zinc-800 opacity-50"
 : skip
 ? "bg-amber-500/30 text-amber-400 border border-amber-500/50 hover:bg-amber-500/40"
 : "bg-zinc-900 text-slate-600 border border-zinc-700 hover:bg-zinc-800 hover:text-slate-700"
 )}
 title={
 isRunningOrQueued 
