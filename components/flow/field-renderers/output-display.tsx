...
function MediaSkeleton({ type, className }: { type: OutputType; className?: string }) {
 ...
 >
 <span className="text-[11px] text-slate-700 dark:text-zinc-600" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
...
function ImageDisplay({ url, className }: { url: string; className?: string }) {
 ...
 return (
 <div className={cn("relative rounded-lg overflow-hidden bg-white dark:bg-white/[0.02]", className)}>
 {!loaded && (
 <div className="absolute inset-0 flex items-center justify-center">
 <Loader2 className="w-6 h-6 text-slate-800 dark:text-zinc-600 animate-spin" />
 </div>
 )}
 <img
 src={url}
...
function AudioDisplay({ url, className }: { url: string; className?: string }) {
 ...
 {/* Volume Icon */}
 <Volume2 className="w-4 h-4 text-slate-800 dark:text-zinc-500 shrink-0" />
...
function TextDisplay({ text, className }: { text: string; className?: string }) {
 ...
 >
 <p className="text-xs text-slate-700 dark:text-zinc-300 whitespace-pre-wrap break-words">
 {displayText}
 </p>
 {isLong && (
 <button
...
function OutputDisplayComponent({
 ...
 <span className="text-[10px] text-slate-700 dark:text-zinc-500">Generating...</span>
 </div>
 );
 }
 ...
 default:
 return (
 <div className={cn("p-3 bg-white dark:bg-white/[0.02] rounded-lg", className)}>
 <pre className="text-[10px] text-slate-800 dark:text-zinc-400 overflow-auto">
