...
function SliderFieldComponent({
 ...
 {config.label && (
 <label className="text-[10px] text-slate-800 dark:text-zinc-500" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
 {config.label}
 {config.required && <span className="text-red-500 ml-0.5">*</span>}
 </label>
 )}
 {config.showValue && (
 <span className="text-[10px] text-slate-700 dark:text-zinc-400 font-mono font-medium">
 {clampedValue}
 </span>
 )}
 </div>
 ...
 <span className="text-[8px] text-slate-800 dark:text-zinc-600">{config.min}</span>
 <span className="text-[8px] text-slate-800 dark:text-zinc-600">{config.max}</span>
 </div>
