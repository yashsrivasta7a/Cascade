...
import type { 
 ...
 WorkflowError, 
 ActivityPanelProps 
} from "./activity-panel/types";
export type { ErrorSeverity, WorkflowError } from "./activity-panel/types";
import {
 getSuggestion,
 formatExactTime,
 formatExactDate,
...
export function ActivityPanel({ 
 ...
 <span className="text-[9px] text-slate-800 dark:text-zinc-600">
 {isCached ? (
 ...
 <p className="text-[10px] text-slate-700 dark:text-zinc-500">Runs & Versions</p>
 </div>
 </div>
 <div className="flex items-center gap-1">
 {activeTab === "runs" && (
 ...
 : "text-slate-800 dark:text-zinc-500 hover:text-red-500 dark:hover:text-red-400"
 )}
 >
 {isDeleting ? (
 ...
 <p className="text-xs text-slate-700 dark:text-zinc-400 mt-1.5">Initializing workflow...</p>
 </div>
 </div>
 </div>
 ) : isMulti ? (
 ...
 <span className="text-[13px] font-medium text-slate-900 dark:text-white truncate">{exec.workflowName || "Workflow Run"}</span>
 ...
 isLatest ? "text-emerald-600 dark:text-emerald-400" : "text-slate-700 dark:text-zinc-400"
 )}>
 {version.version}
 </span>
 </div>
