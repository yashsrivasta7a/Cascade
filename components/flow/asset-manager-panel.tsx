...
function getNodeIcon(nodeType: string) {
  switch (nodeType) {
    case "openrouter": return <Brain className="w-3.5 h-3.5 text-blue-400" />;
    case "seedream": return <Image className="w-3.5 h-3.5 text-emerald-400" />;
    case "seedance": return <Film className="w-3.5 h-3.5 text-violet-400" />;
    case "seedvr": return <Film className="w-3.5 h-3.5 text-violet-400" />;
    case "lipsync": return <Mic className="w-3.5 h-3.5 text-amber-400" />;
    case "elevenlabs": return <Volume2 className="w-3.5 h-3.5 text-amber-400" />;
    case "crop-image": return <Crop className="w-3.5 h-3.5 text-emerald-400" />;
    case "merge-videos": return <Film className="w-3.5 h-3.5 text-violet-400" />;
    case "merge-audio-video": return <Film className="w-3.5 h-3.5 text-violet-400" />;
    case "extract-audio": return <Scissors className="w-3.5 h-3.5 text-amber-400" />;
    default: return <Zap className="w-3.5 h-3.5 text-zinc-400" />;
  }
}

function getNodeColor(nodeType: string): string {
  switch (nodeType) {
    case "openrouter": return "bg-blue-500/15 border-blue-500/30";
    case "seedream": return "bg-emerald-500/15 border-emerald-500/30";
    case "seedance": return "bg-violet-500/15 border-violet-500/30";
    case "seedvr": return "bg-violet-500/15 border-violet-500/30";
    case "lipsync": return "bg-amber-500/15 border-amber-500/30";
    case "elevenlabs": return "bg-amber-500/15 border-amber-500/30";
    case "crop-image": return "bg-emerald-500/15 border-emerald-500/30";
    case "merge-videos": return "bg-violet-500/15 border-violet-500/30";
    case "merge-audio-video": return "bg-violet-500/15 border-violet-500/30";
    case "extract-audio": return "bg-amber-500/15 border-amber-500/30";
          ...
          <div className="px-4 py-3 border-b border-white/[0.06] bg-white/[0.02] shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-purple-500/20 to-purple-600/10 border border-purple-500/20 flex items-center justify-center">
                  <FolderOpen className="w-3.5 h-3.5 text-purple-400" />
                </div>
                <div>
                  <span className="text-sm font-semibold text-white">Asset Manager</span>
                  <p className="text-[10px] text-zinc-500">{mediaAssets.length} file{mediaAssets.length !== 1 ? "s" : ""}</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center bg-white/[0.03] rounded-lg p-0.5 border border-white/[0.04]">
                {(["all", "video", "audio", "image"] as const).map((type) => (
                  <button
                    ...
                    className={cn(
                      "h-6 px-2 text-[9px] font-medium rounded-md flex items-center gap-1 transition-all capitalize",
                      filter === type ? "bg-white/[0.08] text-white" : "text-zinc-500 hover:text-zinc-300"
                    )}
                  >
                    {type === "all" ? "All" : type}
                    {counts[type] > 0 && (
                      <span className={cn(
                        "px-1 py-0.5 text-[7px] font-bold rounded-full min-w-[12px]",
                        type === "video" ? "bg-violet-500/20 text-violet-400" :
                        type === "audio" ? "bg-amber-500/20 text-amber-400" :
                        type === "image" ? "bg-emerald-500/20 text-emerald-400" :
              ...
              {filteredAssets.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-1 h-6 px-2 text-[9px] font-medium text-blue-400 hover:text-blue-300 bg-blue-500/10 hover:bg-blue-500/15 border border-blue-500/20 rounded-lg transition-colors disabled:opacity-50"
                >
          ...
          <div className="flex-1 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-white/10">
            {filteredAssets.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center px-4">
                ...
                <p className="text-sm text-zinc-400">No {filter === "all" ? "assets" : filter + "s"} yet</p>
                <p className="text-xs text-zinc-600 mt-1">Run nodes to generate outputs</p>
              </div>
            ) : (
                ...
                {filteredAssets.map((asset, idx) => (
                  ...
                  >
                    <div className="p-3 flex items-center gap-3">
                      {/* Preview */}
                      <div className="w-14 h-14 rounded-lg bg-black/30 flex items-center justify-center shrink-0 overflow-hidden border border-white/[0.06]">
                        {asset.type === "image" ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <div className={cn("w-5 h-5 rounded-md flex items-center justify-center", getNodeColor(asset.nodeType).split(" ")[0])}>
                            {getNodeIcon(asset.nodeType)}
                          </div>
                          <span className="text-[11px] font-medium text-zinc-200 truncate">{asset.nodeName}</span>
                        </div>
                        <div className="flex items-center gap-2 mt-1.5">
                          {getMediaIcon(asset.type)}
                          <span className="text-[9px] text-zinc-500 capitalize">{asset.type}</span>
                          <span className="text-[9px] text-zinc-600">•</span>
                          <span className="text-[9px] text-zinc-600 truncate">{asset.fieldName}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg text-zinc-500 hover:text-blue-400 hover:bg-blue-500/10 transition-colors disabled:opacity-50"
                          title="Download"
                        >
